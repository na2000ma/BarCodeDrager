...
export class BarCodeComponent implements OnInit {
  ...
  constructor(
    ...
    pageService.pagesObserver.subscribe((pages: Page[]) => {
      this.pages = pages;
    });
    pageService.realPagesObserver.subscribe((pages: Page[]) => {
      this.realPages = pages;
    });
    this.languages = translate.getLangs();

    this.currentLangauge = translate.currentLang;
  }
  ...
  onDropQR(event: any): void {
    ...
    const value = event['dataTransfer'].files[0].name;
    this.isUpload = value ? true : false;
  }
  ...
  onWidthChange(width: any): void {
    ...
    this.imgWidth = width;
    // realpagewidht/pagewidth = realwidth/width => width = (page * realwidth) / realpage
